...
use Otg\Ean\Log\MessageFormatter;
...
class MessageFormatterTest extends \PHPUnit_Framework_TestCase
{
    public function testEnsuresRequestIsMasked()
    {
        ...
        $request = new EntityEnclosingRequest('POST', 'http://foo.com?q=test');
        $request->setBody(EntityBody::factory('<xml><creditCardType>CA</creditCardType>' .
    ...
    {
        $formatter = new MessageFormatter('{url}');

        $request = new EntityEnclosingRequest('POST',
    ...
    {
        $formatter = new MessageFormatter('{resource}');
    ...
    {
        $formatter = new MessageFormatter('{url} - {res_body}');
...
EOD;
    ...
    {
        $formatter = new MessageFormatter('{url} - {response}');
